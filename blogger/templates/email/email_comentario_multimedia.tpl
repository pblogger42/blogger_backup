...
{% block html %}
	<div style="background-color: #f5f5f5; padding: 40px 20px; text-align: center;">
		<div style="background-color: white; margin: 0 auto 10px; width: 80%; padding: 20px 20px; border-radius: 5px 5px; box-shadow: 1px 4px 19px -15px rgba(0,0,0,0.75); text-align: left;">
			...
			<p>El usuario {{ args.escritor.first_name }} {{ args.escritor.last_name }} ha realizado un comentario en el recurso {{ args.multimedia.nombre_multimedia }}</p>
			<p><a href="{{ protocol }}{{ domain }}{% url 'institucion_multimedia' args.multimedia.institucion.slug_institucion %}" class="btn btn-primary" style="background-color: #3498db; padding: 10px 10px; border-radius: 3px 3px; color: white; text-decoration: none; margin-bottom: 10px;">Leer comentario</a></p>
		</div>
